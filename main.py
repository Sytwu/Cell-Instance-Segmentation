...
from models import MaskRCNN, MaskRCNN_v2, MaskRCNN_ConvNeXt, MaskRCNN_Cell
...
def main():
    """Main function to run training, evaluation, and inference."""
    ...
    dataset = InstanceSegmentTrainDataset(cfg.root_dir,
    ...
    print(f"Dataset split: {len(train_set)} training,"
          f" {len(valid_set)} validation samples.")

    train_loader = DataLoader(train_set, batch_size=cfg.batch_size,
                              shuffle=True, collate_fn=collate_fn,
                              num_workers=4, pin_memory=True)
    valid_loader = DataLoader(valid_set, batch_size=cfg.batch_size,
    ...
    if cfg.model_arch == 'MaskRCNN':
        model = MaskRCNN(num_classes=cfg.num_classes,
        ...
        model = MaskRCNN_v2(num_classes=cfg.num_classes,
                            pretrained=cfg.pretrained,
                            rpn_post_nms_train=cfg.rpn_post_nms_train,
                            rpn_post_nms_test=cfg.rpn_post_nms_test,
                            box_detections_per_img=cfg.box_detections_per_img
        ...
        model = MaskRCNN_Cell(num_classes=cfg.num_classes,
                              pretrained=cfg.pretrained,
                              rpn_post_nms_train=cfg.rpn_post_nms_train,
                              rpn_post_nms_test=cfg.rpn_post_nms_test,
                              box_detections_per_img=cfg.box_detections_per_img
    ...
    # 4. Optimizer
    optimizer = model.get_optimizer(base_lr=cfg.base_lr,
                                    weight_decay=cfg.weight_decay)

    # 5. LR Scheduler
    lr_scheduler_obj = None
    if cfg.lr_scheduler == 'step':
        lr_scheduler_obj = lr_scheduler.StepLR(
            optimizer, step_size=cfg.lr_step_size, gamma=cfg.lr_gamma
        )
        print(f"Using StepLR scheduler with step_size={cfg.lr_step_size},"
        ...
        lr_scheduler_obj = lr_scheduler.CosineAnnealingLR(
    ...
    for epoch in range(cfg.num_epochs):
        ...
        valid_loss, valid_mAP = evaluate_model(model, valid_loader, device,
                                               coco_gt)
        ...
        print(f"Epoch {epoch + 1} Summary: Train Loss: {train_loss:.4f} |"
              f" Valid Loss: {loss_str} | Valid mAP: {mAP_str}")
        ...
        if valid_mAP is not None and coco_gt is not None:
            ...
            if current_metric > best_valid_mAP:
                print(
                    f"Validation mAP improved ({best_valid_mAP:.4f} -->"
                    f" {current_metric:.4f}). Saving model..."
                )
            ...
            if current_metric < best_valid_loss:
                print(
                    f"Validation Loss improved ({best_valid_loss:.4f} -->"
                    f" {current_metric:.4f}). Saving model..."
                )
    ...
    print(f"Best Validation mAP: {best_valid_mAP:.4f}"
          if best_valid_mAP != -1.0 else "N/A")
    print(f"Best Validation Loss: {best_valid_loss:.4f}"
          if best_valid_loss != float('inf') else "N/A")
    ...
    try:
        with open(metrics_csv_path, 'w', newline='') as csvfile:
            ...
            csv_writer.writerow(['Epoch', 'Train Loss', 'Valid Loss',
                                 'Valid mAP'])

            for i in range(cfg.num_epochs):
                epoch_num = i + 1
                train_loss = (train_losses[i] if i < len(train_losses)
                              else 'N/A')
                valid_loss = (valid_losses[i] if i < len(valid_losses) and
                              valid_losses[i] is not None else 'N/A')
                valid_mAP = (valid_mAPs[i] if i < len(valid_mAPs) and
                             valid_mAPs[i] != -1 else 'N/A')
                csv_writer.writerow([epoch_num, train_loss, valid_loss,
                                     valid_mAP])
    ...
    try:
        ...
        plt.subplot(1, 2, 2)
        valid_mAPs_plot = [m if m is not None and m != -1 else np.nan
                           for m in valid_mAPs]
        if not all(np.isnan(m) for m in valid_mAPs_plot):
            plt.plot(range(1, cfg.num_epochs + 1), valid_mAPs_plot,
                     label='Valid mAP', marker='o')
            plt.xlabel('Epoch')
            ...
            plt.text(0.5, 0.5, 'mAP not calculated',
    ...
    if os.path.exists(best_model_path):
        ...
        inference(model, cfg.test_dir, cfg.test_json, device,
                  output_json_path=inference_output_path)
    else:
